...
class Transaksi extends CI_Controller {
	...
	{	
		$this->data['title_web'] = 'Data Pinjam tenant ';
	...
	{	
		$this->data['title_web'] = 'Data Pengembalian tenant ';
		$this->data['idbo'] = $this->session->userdata('ses_id');
	...
	{	
		...
		$this->data['title_web'] = 'Tambah Pinjam tenant ';
	...
	public function detailpinjam()
	{
		...
		$this->data['title_web'] = 'Detail Pinjam tenant ';
		$this->load->view('header_view',$this->data);
		$this->load->view('sidebar_view',$this->data);
	...
	public function kembalipinjam()
	{
		...
		$this->data['title_web'] = 'Kembali Pinjam tenant ';
		$this->load->view('header_view',$this->data);
		$this->load->view('sidebar_view',$this->data);
	...
	public function prosespinjam()
	{
		...
		if(!empty($post['tambah']))
		{
			...
			<p> Tambah Pinjam tenant Sukses !</p>
			</div></div>');
		...
		if($this->input->get('pinjam_id'))
		{
			...
			<p>  Hapus Transaksi Pinjam tenant Sukses !</p>
			</div></div>');
			redirect(base_url('transaksi')); 
		}
		...
		if($this->input->get('kembali'))
		{
			...
			$this->session->set_flashdata('pesan','<div id="notifikasi"><div class="alert alert-success">
			<p> Pengembalian Pinjam tenant Sukses !</p>
			</div></div>');
			redirect(base_url('transaksi')); 
    ...
    {	
		...
		$row = $this->db->query("SELECT * FROM tbl_tenant WHERE tenant_id ='$id'");
	...
	?>
		<table class="table table-striped">
			...
			<?php $no=1;
				...
				<tr>
					...
					<td style="width:17%">
					<a href="javascript:void(0)" id="delete_tenant<?=$no;?>" data_<?=$no;?>="<?= $items['id'];?>" class="btn btn-danger btn-sm">
						<i class="fa fa-trash"></i></a>
					</td>
				</tr>
				<script>
					$(document).ready(function(){
						$("#delete_tenant<?=$no;?>").click(function (e) {
							$.ajax({
								type: "POST",
								url: "<?php echo base_url('transaksi/del_cart');?>",
								data:'kode_tenant='+$(this).attr("data_<?=$no;?>"),
								beforeSend: function(){
								},
		...
		<?php foreach(array_values(unserialize($this->session->userdata('cart'))) as $items){?>
			<input type="hidden" value="<?= $items['id'];?>" name="idtenant[]">
		<?php }?>
		<div id="tampil"></div>
	<?php
	}
	...
	public function del_cart()
    {
       ...
       // redirect('jual/tambah');
		echo '<script>$("#result_tenant").load("'.base_url('transaksi/tenant_list').'");</script>';
    }
