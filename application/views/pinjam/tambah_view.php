							...
							<div class="col-sm-7">
								<table class="table table-striped">
									...
									<tr>
										...
										<td>
											<div class="input-group">
												<input type="text" class="form-control" autocomplete="off" name="tenant_id" id="tenant-search" placeholder="Contoh ID tenant : BK001" type="text" value="">
												<span class="input-group-btn">
													<a data-toggle="modal" data-target="#Tabletenant" class="btn btn-primary"><i class="fa fa-search"></i></a>
												</span>
											</div>
										</td>
									</tr>
									<tr>
										...
										<td>:</td>
										<td>
											<div id="result_tunggu_tenant"> <p style="color:red">* Belum Ada Hasil</p></div>
											<div id="result_tenant"></div>
										</td>
									</tr>
								</table>
							</div>
...
<!--modal import -->
<div class="modal fade" id="Tabletenant">
...
<table id="example1" class="table table-bordered table-striped">
		...
		<?php $no=1;foreach($tenant->result_array() as $isi){?>
			<tr>
				...
				<td><?= $isi['tgl_masuk'];?></td>
				<td style="width:17%">
				<button class="btn btn-primary" id="Select_File2" data_id="<?= $isi['tenant_id'];?>">
					<i class="fa fa-check"> </i> Pilih
				</button>
				<a href="<?= base_url('data/tenantdetail/'.$isi['id_tenant']);?>" target="_blank">
...
<script>
	$(".fileSelection1 #Select_File2").click(function (e) {
		document.getElementsByName('tenant_id')[0].value = $(this).attr("data_id");
		$('#Tabletenant').modal('hide');
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('transaksi/tenant');?>",
			data:'kode_tenant='+$(this).attr("data_id"),
			beforeSend: function(){
				$("#result_tenant").html("");
				$("#result_tunggu_tenant").html('<p style="color:green"><blink>tunggu sebentar</blink></p>');
			},
			success: function(html){
				$("#result_tenant").load("<?= base_url('transaksi/tenant_list');?>");
				$("#result_tunggu_tenant").html('');
			}
		});
	});
	</script>
